...
defined in the <tt>salesOrder</tt> [resource kind](1.1 Terminology "1.1 Terminology")
...
query parameter is set in the URL (see [Query
Parameters](2.11 Query Parameters "2.11  Query Parameters") section).
*   operations allowed on property URLs. POST, PUT and DELETE are only allowed
on child resources (see [Resource Property URL](2.3 Resource Property URL "2.3 Resource Property URL")
section).
*   synchronization: child resources are synchronized together with their
parent, they are not synchronized separately (see
[Relationship Handling in Synchronization](6.1 Relationship Handling in Synchronization "6.1 Relationship Handling in Synchronization")).
...
[denormalized data on
Wikipedia](http://en.wikipedia.org/wiki/Denormalization)). This is best explained with an example:
...
allowed operations. See [Resource Property URL
section](2.3 Resource Property URL "2.3 Resource Property URL").

## Advertising paging modes
...
returned feed. See [Query Paging section](6.4 Query Paging "6.4 Query Paging"). For example:
