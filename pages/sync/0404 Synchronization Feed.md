...
?startIndex=11&amp;amp;count=10" /&gt;
  ...
  &lt;!-- [Digest](/daisy/sdata/171-DSY/g4/173-DSY.html) --&gt;
...
The target will use this digest to detect conflicts (see section on&nbsp;[Conflict Handling with Digest](/daisy/sdata/171-DSY/g2/244-DSY.html)).
...
standard&nbsp;[paging
links](6.4 Query Paging)&nbsp;<tt>(&lt;link rel="next"&gt;</tt>) if the synchronization feed is
broken in pages.
...
provide as response to operations on the <tt>$syncSource</tt> and
<tt>$syncTarget</tt> operations a feed structured in the following manner:

*   <tt>syncMode</tt> specification: MUST for <tt>$syncTarget</tt>
request

*   digest: MUST for <tt>$syncSource</tt> and <tt>$syncTarget&nbsp;</tt>

*   requests and responses entry (sequence): MUST for
<tt>$syncTarget</tt> request

With regard to the sequence of entries in the feed, the entries
for a given endpoint MUST be ordered by ascending tick.

The provider MUST support sequential paging mechanism for
