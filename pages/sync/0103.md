...
<p class="compliance">A provider supporting linking MUST support the creation of a
linking entry to a previously unlinked resource. This is achieved by the POST
operation on the <tt>$linked</tt> URL.</p>

<p class="compliance">If the payload contains a <tt>sdata:uuid</tt> attribute, the
...
<p class="compliance">A provider SHOULD accept a POST operation on a resource that is
...
<p class="compliance">However, a provider MUST reject a POST request targeting an
...
<p class="compliance">The provider MUST provide the <tt>sdata:uuid</tt> attribute in
the response. Its value MUST identify the resource targeted by the POST
operation.</p>

<p class="compliance">The initiator of the POST request on the <tt>$linked</tt> URL
